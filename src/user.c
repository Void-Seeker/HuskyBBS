...
void memory_other( USER_DATA *usr )
{
    ...
    sprintf(buf, "User : %4d (%8zu bytes), %2d free (%zu bytes)\n\r",
            count1, count1 * (sizeof(*fusr)), count2, count2 * (sizeof(*fusr)));
    send_to_user(buf, usr);

    sprintf(buf, "Mesg : %4d -- %zu bytes\n\r", count_msg,
            count_msg * (sizeof(*fMessage)));
    send_to_user(buf, usr);

    sprintf(buf, "Mail : %4d -- %zu bytes\n\r", count_mail,
    ...
    sprintf(buf, "Desc : %4d (%8zu bytes), %2d free (%zu bytes)\n\r",
